<template>
  ...
  <div class="page" v-if="profile">
    ...
    <div class="card" style="max-width:560px">
      ...
      <span class="badge" :class="profile.status === 'Blacklisted' ? 'badge-red' : 'badge-green'" style="margin-left:6px">
        {{ profile.status }}
      </span>
      ...
      <div class="actions" style="margin-top:18px">
        <router-link class="btn btn-blue" to="/edit_profile">Edit Profile</router-link>
        <router-link class="btn btn-gray" to="/user/dashboard">Back to Dashboard</router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async load() {
      ...
      try {
        this.profile = await api.get(`/profile/${u.id}`);
      } catch (e) {
